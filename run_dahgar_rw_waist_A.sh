...
for patch in 8 ; do
    for stride in  8 ; do
        python run_LLM4TS.py \
            ...
            --is_training 1 \
            --root_path /workspaces/HIAAC-KR-Dev-Container/some_datasets/DAGHAR/standardized_view/RealWorld_waist/ \
            --model_id LLM4TS_cls_RealWorld_waist \
            --model LLM4TS_cls \
            --data DAGHAR \
            --is_llm 1 \
            --pretrain 1 \
            ...
            --checkpoints ./checkpoints/classification/RealWorld_waist \
            --des exp \
            --lradj type1 \
            --use_gpu 1 \
            --devices 0 \
            --gpu 0 \
            --num_workers 0 2>&1 | tee logs/classification/DAGHAR-RealWorld_waist_patch-${patch}_stride-${stride}.log 
    done
done