...
def get_module_order(model):
    return {module: i for i, (_, module) in enumerate(model.named_modules())}

from collections import defaultdict
import json
module_order = None
global_iter = 0
global_grads = defaultdict(dict)

def print_mean_gradient_hook(module, grad_input, grad_output):
    global global_grads, module_order, global_iter
    if grad_output[0] is not None:
        order = module_order.get(module, -1)
        mean_grad = grad_output[0].mean().item()
        global_grads[int(global_iter)][int(order)] = {"name": module.__class__.__name__, "mean_grad": mean_grad}
        print(f"[{order:02d}] {module.__class__.__name__}: Mean grad = {mean_grad:.6f}")
        

        
def register_hooks(model):
    for name, module in model.named_modules():
        module.register_full_backward_hook(print_mean_gradient_hook)
...
class Exp_Classification(Exp_Basic):
    ...
    def _build_model(self):
        ...
        global module_order
        module_order = get_module_order(model)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, label, padding_mask) in tqdm.tqdm(
                ...
                iter_count += 1
                global_iter += 1
                print(f"---- Global iter: {global_iter} ----")
        ...
        # Save global_grads to a JSON file
        grads_save_path = os.path.join(path, "global_grads.json")
        with open(grads_save_path, "w") as f:
            json.dump(global_grads, f, indent=4, sort_keys=True)
        print(f"Global gradients saved to {grads_save_path}")
