...
class Exp_Main(Exp_Basic):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if "LLM4TS_sft_zero" == self.args.model:
                    ...
                    if self.args.rand_init:
                        batch_x_ = torch.randn_like(batch_y).to(
                    ...
                    if self.args.fft:
                        ...
                        indices = np.argsort(np.abs(fft_result))[::-1][
                            :, :, -num_components:
                        ]
                        fft_result_filtered = np.zeros_like(fft_result)
                        fft_result_filtered[
                            np.arange(fft_result_filtered.shape[0])[
                                :, None, None
                            ],
                            np.arange(fft_result_filtered.shape[1])[:, None],
                            indices,
                        ] = fft_result[
                            np.arange(fft_result_filtered.shape[0])[
                                :, None, None
                        ...
                        batch_x_ = (
                            torch.tensor(reconstructed_signal)
                ...
                if "LLM4TS_pt" == self.args.model:
                    ...
                    batch_y = batch_y.unfold(
                        dimension=-1,
                        size=self.args.patch_len,
                        step=self.args.stride,
                    )
                elif "LLM4TS_sft_zero" == self.args.model:
                    num_loss_patch = (
                        self.args.pred_len - self.args.patch_len
                    ) // self.args.stride + 1
    ...
    def train(self, setting):
        ...
        if "LLM4TS_sft_zero" == self.args.model:
            summary(
                self.model,
                input_size=[x[:, -self.args.history_len :, :].size(), y.size()],
            )
        else:
        ...
        for epoch in range(self.args.train_epochs):
                ...
                if self.args.use_amp:
                    ...
                    if "LLM4TS_sft_zero" == self.args.model:
                        ...
                        if self.args.fft:
                            ...
                            fft_result_filtered[
                                ...
                                np.arange(fft_result_filtered.shape[1])[
                                    :, None
                                ],
                                indices,
                            ] = fft_result[
                    ...
                    if "LLM4TS_sft_zero" == self.args.model:
                        ...
                        if self.args.data_path == "illness.csv":
                            for kind in [
                                (24 - self.args.patch_len) // self.args.stride
                                + 1,
                                (36 - self.args.patch_len) // self.args.stride
                                + 1,
                                (48 - self.args.patch_len) // self.args.stride
                                + 1,
                                (60 - self.args.patch_len) // self.args.stride
                                + 1,
                            ]:
                                loss += criterion(
                                    outputs[:, :, :kind, :],
                            ...
                            for kind in [
                                (96 - self.args.patch_len) // self.args.stride
                                + 1,
                                (192 - self.args.patch_len) // self.args.stride
                                + 1,
                                (336 - self.args.patch_len) // self.args.stride
                                + 1,
                                (720 - self.args.patch_len) // self.args.stride
                                + 1,
                            ]:
            ...
            ckpt_path = path + "/" + f"checkpoint_epoch_{epoch+1}.ckpt"
    ...
    def test(self, setting, test=0):
        ...
        if test:
            ...
            self.model.load_state_dict(
                torch.load(
                    os.path.join(
                        "./test_ckpt/" + setting[:-2], "checkpoint.pth"
                    )
                )
            )
                ...
                if "LLM4TS_pt" == self.args.model:
                    ...
                    for b in range(B):
                        for c in range(C):
                            ...
                            for i in range(n):
                                ...
                                if last_end <= start:
                                    y_hat[b, c, start:end] += outputs[
                                        b, c, i, :
                                    ]
                                else:
                                    y_hat[b, c, start:last_end] = (
                                        y_hat[b, c, start:last_end]
                                        + outputs[b, c, i, : (last_end - start)]
                                    ) / 2
                                    y_hat[b, c, last_end:end] += outputs[
                                        b, c, i, (last_end - start) :
                                    ]
                                last_end = end
                    outputs = y_hat
                    ...
                    for j in range(n):
                        ...
                        if last_end <= start:
                            ...
                            y_hat[:, :, start:last_end] = (
                                y_hat[:, :, start:last_end]
                                + outputs[:, :, j, : (last_end - start)]
                            ) / 2
                            y_hat[:, :, last_end:end] += outputs[
                                :, :, j, (last_end - start) :
                            ]
                        last_end = end
        ...
        if "LLM4TS_sft_zero" == self.args.model:
            ...
            if self.args.data_path == "illness.csv":
                for pred_len_ in [24, 36, 48, 60]:
                    mae, mse, rmse, mape, mspe, rse = metric(
                        preds[:, :pred_len_, :], trues[:, :pred_len_, :]
                    )
                    print(
                        "{}: mse:{}, mae:{}, rse:{}".format(
                            pred_len_, mse, mae, rse
                        )
                    )
            else:
