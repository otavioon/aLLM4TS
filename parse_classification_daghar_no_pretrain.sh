...
# Get arguments
log_dir="${1:-logs/classification_daghar_no_pretrain}"
output_csv="${2:-allm4ts_results_daghar_no_pretrain.csv}"
tmp_dir=$(mktemp -d)
...
python convert_csv_to_analysis.py --input_csv ${tmp_file} --output_csv ${output_csv} --backbone "GPT" --tsk_pretext "GPT" || die "Failed to convert CSV to analysis format"
