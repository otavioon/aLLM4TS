...
for patch in 8 ; do
    for stride in  8 ; do
        python run_LLM4TS.py \
            ...
            --checkpoints ./checkpoints/classification/KuHar \
            --des exp \
            --lradj type1 \
            --use_gpu 1 \
            --devices 0 \
            --gpu 0 \
            --num_workers 0 2>&1 | tee logs/classification/DAGHAR-KuHar_patch-${patch}_stride-${stride}.log 
    done
done