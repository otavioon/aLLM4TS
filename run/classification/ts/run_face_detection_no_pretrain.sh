    ...
    for patch in 16 ; do
        for stride in 16 ; do
            python run_LLM4TS.py \
                ...
                --model_id LLM4TS_cls_FaceDetection \
                --model LLM4TS_cls \
                --data UEA \
                --is_llm 1 \
                --pretrain 1 \
                ...
                --sft 1 \
                --sft_layers ln_wpe \
                --checkpoints ./checkpoints/classification_no_pretrain/FaceDetection \
                --des exp \
                --lradj type1 \
                --use_gpu 1 \
                --devices 0 \
                --gpu 0 \
                --num_workers 0 2>&1 | tee logs/classification_no_pretrain/FaceDetection_patch-${patch}_stride-${stride}_lr-${lr}.log
        done
    done
done