    ...
    for patch in 8 ; do
        for stride in 4 ; do
            python run_LLM4TS.py \
                ...
                --sft 1 \
                --sft_layers ln_wpe \
                --checkpoints ./checkpoints/classification_no_pretrain/Handwriting \
                --des exp \
                --lradj type1 \
                --use_gpu 1 \
                --devices 0 \
                --gpu 0 \
                --num_workers 0 2>&1 | tee logs/classification_no_pretrain/Handwriting_patch-${patch}_stride-${stride}_lr-${lr}.log
        done
    done
done
