...
for patch in 8 16 ; do
    for stride in 16 8 4 2 ; do
        python run_LLM4TS.py \
            ...
            --checkpoints ./checkpoints/classification/EthanolConcentration \
            --des exp \
            --lradj type1 \
            --use_gpu 1 \
            --devices 0 \
            --gpu 0 \
            --num_workers 0 2>&1 | tee logs/classification/EthanolConcentration_patch-${patch}_stride-${stride}.log 
    done
done