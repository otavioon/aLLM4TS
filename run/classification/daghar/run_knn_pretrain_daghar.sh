...
mkdir -p logs/classification_knn
                    ...
                    for freeze in 0 ; do
                        for n_neighbors in 2 3 5 7 10 ; do
                            ...
                            python run_LLM4TS.py \
                                ...
                                --checkpoints ./checkpoints/classification_knn/${dataset}_patch-${patch}_stride-${stride}_aLLM4TS-${pt_code}_norm-${instance_norm}_percent-${percent}_freeze-${freeze}_head-${head} \
                                --des exp \
                                --lradj type1 \
                                --use_gpu 1 \
                                --devices 0 \
                                ...
                                --num_workers 0 2>&1 | tee logs/classification_knn/${dataset}_patch-${patch}_stride-${stride}_aLLM4TS-${pt_code}_norm-${instance_norm}_percent-${percent}_freeze-${freeze}_head-${head}_neighbors-${n_neighbors}.log
                        done
                    done
                done
            done
