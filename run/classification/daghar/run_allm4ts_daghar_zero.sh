...
mkdir -p logs/classification_daghar_zero
                ...
                for percent in 1 10 50 100 ; do
                    ...
                    for freeze in 0 2 ; do
                        ...
                        python run_LLM4TS.py \
                            ...
                            --checkpoints ./checkpoints/classification_daghar_zero/${dataset}_patch-${patch}_stride-${stride}_aLLM4TS-${pt_code}_norm-${instance_norm}_percent-${percent}_freeze-${freeze}_head-${head} \
                            --des exp \
                            --lradj type1 \
                            --use_gpu 1 \
                            --devices 0 \
                            ...
                            --percent ${percent} \
                            --head ${head} \
                            --num_workers 0 2>&1 | tee logs/classification_daghar_zero/${dataset}_patch-${patch}_stride-${stride}_aLLM4TS-${pt_code}_norm-${instance_norm}_percent-${percent}_freeze-${freeze}_head-${head}.log
                    done
                done
            done
        done
